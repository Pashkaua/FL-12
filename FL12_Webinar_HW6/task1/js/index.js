...
$("#header").next("div").css({
  "border": "2px solid blue", "outline": "2px solid red "
});
...
$("#list-1").children(":not(.cloned)").css({ "display": "block" });

// hide empty li

$("#list-1").children(":empty").css({ "display": "none" });
...
$("#list-3").children(":first").css({ "color": "green" });
...
$("#list-3:has(em)").css({ "color": "red" });

// for li which contains text 'Buratino' set font weight to bold

$(".list").children(":contains(Buratino)").css({ "font-weight": "bold" });
...
$("img").parent("div").css({ "border": "1px solid red", "float": "left" });
